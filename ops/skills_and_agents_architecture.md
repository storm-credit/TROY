...
- 에이전트별 모델 최적화와 reasoning 강도 배정도 `총괄 오케스트라`
...
- 어떤 전문가를 붙일지와 어떤 모델로 돌릴지는 에이전트가 아니라 총괄이 정한다
- 같은 전문가라도 작업 리스크가 바뀌면 총괄이 mini / standard / higher reasoning을 다시 고른다
- 에이전트는 배정된 모델과 reasoning 범위 안에서만 판단한다
