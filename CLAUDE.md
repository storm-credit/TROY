...
## 18. 낭독 검수 프로토콜 (2026-08-06 등록 — 화별 라인에디트 표준)

재사용 프롬프트: **`ops/reading_review_prompt.md`** (한 화 낭독 검수 시 붙여넣어 사용). 착수 전 캐논 맹점 스윕 → 함정 목록 보고 → 5축 판정 → 로그 표 → 적용 → 게이트 → 커밋.

**8축**: ①공감각 정확성(감정=음악은 서준 고유 지각·능력=끝까지 비밀 world §10) ②연재체 완화(줄바꿈 파편→산문, 메모/수첩/톡방 backtick 유지) ③음악비유 밀도 규율(박자·리듬·선율 **화당 3~4회 상한**, 같은 문단 연속 금지, 디에제틱 제외) ④종결·감정 문법(house bible §6 잠언/슬로건/제목콜백 종결 금지, 결말 문법 불변) ⑤자연스러움(은유 사실 착지 + 호칭 캐논: 서준↔아린 존댓말·↔이든 반말) ⑥원천 3작 질감(소품이 감정을 짐·아린 미화 금지·시라노 아이러니) ⑦음악 현실착지+문체 앵커(최은영·김애란·이도우, house bible §9) ⑧**AI 지문 제거**(삼연속 병렬 공식·헤지 남발·모호 부사·정의문 버릇·부정 대구·축소 수식어·문단 착지 강박·반응 공식 — 화 내부 + 화간 코퍼스 레벨, 체크리스트는 reading_review_prompt ⑧).

**운영 정책 (사용자 확정)**:
- **엔딩 정책 A**: 작가가 준 "기존"이 현재 원고에 없으면(= Phase 2 절삭 꼬리) 자동 보류, 매칭분만 적용. 절삭분 기본 유지.
- **반복 봉합 규칙**: 수정이 *안 고친 옆 문장과 부딪혀 만든 반복*만 봉합. 작가 어휘 선택은 존중(과교정 금지).
- **연재체→산문 재조판**: 줄바꿈 파편 문단은 산문으로 병합(자수 감소는 ±10% 예외로 인정).
