...
> ⚠ .bat / .vbs 파일은 인코딩 문제 회피를 위해 **순수 ASCII 영어**로 작성되어 있음. 한국어 안내는 본 README.md 만. Korean Windows 의 cmd 가 UTF-8 .bat 을 CP949 로 잘못 읽어 한글이 깨지는 문제를 회피.
...
2. 자동 진행:
   ...
   - 바탕화면에 `TROY-Control\` 폴더 + 4개 제어 파일 복사
...
| 파일 | 동작 | 창? |
|---|---|---|
| `TROY-Start.vbs` | 서비스 시작 | ❌ 무창 |
| `TROY-Stop.vbs` | 서비스 정지 | ❌ 무창 |
| `TROY-Restart.vbs` | 서비스 재시작 (코드 업데이트 후 등) | ❌ 무창 |
| `TROY-Status.bat` | 상태 + HTTP 응답 + 최근 로그 표시 | ✅ 표시 |
...
### 코드 변경 후 즉시 반영 안 됨
Next.js dev mode 는 일반적으로 hot-reload 됨. 안 되면 `TROY-Restart.vbs`.

### 첫 설치 시 한글 깨짐 + powershell 명령 에러 (v1 이슈)
**해결 완료** — .bat 파일을 ASCII 영어로 재작성. 현재 버전은 안전.
v1 의 한글 / 박스문자 (━) 가 cmd 의 ANSI 코드페이지 (CP949) 와 충돌해
`powershell` 이 `ershell` 로 잘리는 등의 문제 발생했었음.
